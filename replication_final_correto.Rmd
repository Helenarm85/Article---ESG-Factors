...
obter_empresas_esg_por_data <- function(col_alvo, data_ref, lista_sheets) {
  ...
  corte_minimo <- qtd_fundos_ativos_data * 0.20  #1% ou 20% (Change the values here for 0.00001 for Y>=1 and 0.2 for Y>= 20%)
...
Creating the specific dataset df_sem_outliers for robust models.
```{r include=FALSE}
...
cat("Rows after Winsorization:", nrow(df_sem_outliers), "(Should be identical)\n")
```
...
modelo_pooled_full <- glm(Empresa_ESG ~ Dividend_Yield + Debt_Assets + ROA + ROE + 
                            Gross_Margin + EBITDA_Margin + Net_Margin + Total_Assets + Dummy_Tem_ESG, 
                          data = df_regressao_limpo_sem_na, 
                          family = binomial(link = "logit"))
summary(modelo_pooled_full)

# --- Model 2: Pooled Logit with ESG Dummy (Outliers Removed) ---
# Adds the Dummy_Tem_ESG variable
modelo_pooled_esg_sem_outliers <- glm(Empresa_ESG ~ Dividend_Yield + Debt_Assets + ROA + ROE + 
                                        Gross_Margin + EBITDA_Margin + Net_Margin + Total_Assets + 
                                        Dummy_Tem_ESG, 
                                      data = df_sem_outliers, 
                                      family = binomial(link = "logit"))
summary(modelo_pooled_esg_sem_outliers)
...
modelo_painel_full <- feglm(Empresa_ESG ~ Dividend_Yield + Debt_Assets + ROA + ROE + 
                       ...
                       data = df_regressao_limpo_sem_na, # Using df_final or df_sem_outliers based on preference
...
# The most robust specification: Fixed Effects for Firm and Time (Date)
# Uses log(Total_Assets) for scaling
modelo_painel_sem_outliers <- feglm(Empresa_ESG ~ Dividend_Yield + Debt_Assets + ROA + ROE + 
                         Gross_Margin + EBITDA_Margin + Net_Margin + log(Total_Assets) + 
                         Dummy_Tem_ESG | Nome + Data,
                       data = df_sem_outliers, # Using df_final or df_sem_outliers based on preference
                       family = binomial("logit"))

summary(modelo_painel_sem_outliers)
...
# --- Model 2: Pooled Logit with ESG Dummy (Outliers Removed) ---
# Adds the Dummy_Tem_ESG variable
modelo_pooled_esg_sem_outliers_probit <- glm(Empresa_ESG ~ Dividend_Yield + Debt_Assets + ROA + ROE + 
                                        Gross_Margin + EBITDA_Margin + Net_Margin + Total_Assets + 
                                        Dummy_Tem_ESG, 
                                      data = df_sem_outliers, 
                                      family = binomial(link = "probit"))
summary(modelo_pooled_esg_sem_outliers_probit)
...
# The most robust specification: Fixed Effects for Firm and Time (Date)
# Uses log(Total_Assets) for scaling
modelo_painel_sem_outliers_probit <- feglm(Empresa_ESG ~ Dividend_Yield + Debt_Assets + ROA + ROE + 
                         Gross_Margin + EBITDA_Margin + Net_Margin + log(Total_Assets) + 
                         Dummy_Tem_ESG | Nome + Data,
                       data = df_sem_outliers, # Using df_final or df_sem_outliers based on preference
                       family = binomial("probit"))

summary(modelo_painel_sem_outliers_probit)

```

```{r results='asis'}
modelsummary(
  title = "Pooled Logit Model: Entry Criterion (Y ≥ 20%) ",
  modelo_pooled_full, 
  output = "latex",
...
modelsummary(
  title = "Pooled Logit Model: Entry Criterion (Y ≥ 20%) - Winsorized",
  modelo_pooled_esg_sem_outliers, 
  output = "latex",
  # The formula 'term ~ statistic' forces stats into columns
  shape = term ~ statistic, 
  statistic = c("Std. Error" = "{std.error}", 
...
modelsummary(
  title = "Fixed Effects Logit Estimation: Entry Criterion (Y ≥ 20%)",
  modelo_painel_full, 
  output = "latex",
  # The formula 'term ~ statistic' forces stats into columns
  shape = term ~ statistic, 
...
modelsummary(
  title = "Fixed Effects Logit Estimation: Entry Criterion (Y ≥ 20%) - Winsorized",
  modelo_painel_sem_outliers, 
  output = "latex",
  # The formula 'term ~ statistic' forces stats into columns
  shape = term ~ statistic, 
  statistic = c("Std. Error" = "{std.error}", 
...
modelsummary(
  title = "Pooled Logit Model: Entry Criterion (Y ≥ 20%) - Winsorized",
  modelo_pooled_esg_sem_outliers_probit, 
  output = "latex",
  # The formula 'term ~ statistic' forces stats into columns
  shape = term ~ statistic, 
  statistic = c("Std. Error" = "{std.error}", 
...
modelsummary(
  title = "Fixed Effects Logit Estimation: Entry Criterion (Y ≥ 20%) - Winsorized",
  modelo_painel_sem_outliers_probit, 
  output = "latex",
  # The formula 'term ~ statistic' forces stats into columns
  shape = term ~ statistic, 
  statistic = c("Std. Error" = "{std.error}", 
