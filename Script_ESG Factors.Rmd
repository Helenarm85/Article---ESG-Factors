...
# Logic: Checks fund holdings for a specific date and applies x% cutoff rule
obter_empresas_esg_por_data <- function(col_alvo, data_ref, lista_sheets) {
  ...
  # Aqui a função retorna os dados que você precisa lá fora
  return(list(
    data = list_rbind(lista_processada),
    qtd = length(lista_processada)
  ))
}
  
  # Calculate frequency of investment in stocks
  definir_esg_list <- function(ativos_data, qtd_fundos_ativos_data, data_ref, percentual) { 
  ranking <- ativos_data %>%
    filter(`Tipo de investimento` == "Ações") %>%
    distinct(Nome_Fundo, Descrição) %>%
    count(Descrição, name = "Qtd_Fundos_Investindo", sort = TRUE)
  
  # Cutoff rule (1% ou 20% of active funds in that month)
  corte_minimo <- qtd_fundos_ativos_data * percentual 
  #0.20  #1% ou 20% (Change the values here for 0.00001 for Y>=0.01 and 0.2 for Y>= 1)
...
resultados_consolidados <- mapa_colunas_datas %>%
  pmap(function(coluna_original, data_extraida) {
    ...
    # A) Extrai os dados puros para aquela data específica
    temp <- obter_empresas_esg_por_data(coluna_original, data_extraida, all_sheets)
    
    if (is.null(temp)) return(NULL)
    
    # B) Aplica a função de corte dinâmico (Aqui você pode alterar entre 0.20 e 0.01)
    percentual_escolhido <- 0.20 # MUDE AQUI PARA 0.01 SE QUISER O CENÁRIO DE 1%
    
    resultado_corte <- definir_esg_list(temp$data, temp$qtd, data_extraida, percentual_escolhido)
    
    # C) Retorna a estrutura EXATA que o restante do relatório (map_chr e Chunk 5) exige
    return(list(
      data = data_extraida,
      vetor_nomes = resultado_corte$vetor_nomes
    ))
...
dados_esg <- read_excel("ESGscore.Refinitiv.xlsx")
...
all_sheets <- purrr::map(sheet_names, function(x) {
...
# 4. Integrate Refinitiv ESG Scores
df_regressao_atualizado <- read_excel("ESGscore.Refinitiv.xlsx") 
df_regressao_atualizado$ESGScore[df_regressao_atualizado$ESGScore == "NULL"] <- NA
df_regressao_atualizado <- df_regressao_atualizado %>% mutate(Dummy_Tem_ESG = ifelse(!is.na(ESGScore), 1, 0))

df_regressao_atualizado <-
  df_regressao_atualizado |> 
  separate(Código, c("Código", "other"), "\\.") |> 
  distinct(Código, Ano_Referencia, Dummy_Tem_ESG)

## check
anyNA(df_regressao_atualizado)

df_final <-
  df_regressao |>
  mutate(Ano_Referencia = lubridate::year(Data)) |>
  left_join(df_regressao_atualizado) |>
  replace_na(list(Dummy_Tem_ESG = 0)) |>
  select(-"Ano_Referencia")

# df_final <- readRDS("df_final_limpo_v2.rds")
...
```
  ...
  # --- SEÇÃO 7 & 8: TRATAMENTO DE DADOS ---
  8. Winsorized (Strict)
```{r}
table(df_final$Dummy_Tem_ESG)

# 1. Definição da função de Winsorização (Faça isso no início do script)
...
# 2. Criação da Base Higienizada (Sua "Base de Comparação")
df_limpa_sem_wins <- df_regressao %>%
  filter(!Código %in% c("SYNE3", "AMAR3", "RCSL3")) %>%
  filter(EBITDA_Margin > -500 & EBITDA_Margin < 500) %>%
  filter(ROE > -500 & ROE < 500) %>%
  filter(is.finite(Debt_Assets), is.finite(ROA)) %>%
  mutate(log_Total_Assets = log(Total_Assets))

# 3. Criação da Base Oficial (Sua "Base Final Winsorizada")
# Agora, aplicamos a transformação apenas uma vez sobre a base já limpa
df_final <- df_limpa_sem_wins %>%
  mutate(across(all_of(colunas_numericas), ~ winsorize_vector(.))) %>%
  mutate(log_Total_Assets = log(Total_Assets)) # Atualiza o log com os valores winsorizados

# 4. Verificação Final (Para ter certeza que funcionou)
cat("Linhas na base limpa:", nrow(df_limpa_sem_wins), "\n")
cat("Linhas na base final:", nrow(df_final), "\n")
cat("Máximo ROE (Final):", max(df_final$ROE, na.rm=T), "\n")
...
modelo_pooled_full <- glm(Empresa_ESG ~ Dividend_Yield + Debt_Assets + ROA + ROE + 
                            Gross_Margin + EBITDA_Margin + Net_Margin + log_Total_Assets + Dummy_Tem_ESG, 
                          data = df_final, 
...
modelo_pooled_esg_winsorized <- glm(Empresa_ESG ~ Dividend_Yield + Debt_Assets + ROA + ROE + 
                                        Gross_Margin + EBITDA_Margin + Net_Margin + log_Total_Assets + 
                                        Dummy_Tem_ESG, 
                                      data = df_winsorized, 
                                      family = binomial(link = "logit"))
...
modelo_painel_full <- feglm(Empresa_ESG ~ Dividend_Yield + Debt_Assets + ROA + ROE + 
                       ...
                       data = df_final, # Using df_final or df_winsorized based on preference
...
# install.packages(c("psych", "corrplot", "car", "Hmisc"))

# Carregar as bibliotecas
library(psych)    # Para estatísticas descritivas detalhadas
...
## applying desc stats on final data, change if needed.

data_for_desc_stat <-
  df_final %>% 
  select(where(is.numeric)) 
  #select(-"Dummy_Tem_ESG", -"Ano_Referencia", -"std_dev_3y_PL")

## Basic summary - Statistic discriptive and Correlation matrix
# 1. LOG TOTAL ASSETS (FAÇA PRIMEIRO)
data_for_desc_stat$log_Total_Assets <- log(data_for_desc_stat$Total_Assets + 1)
data_for_desc_stat$Total_Assets <- NULL

# 2. ESTATÍSTICA DESCRITIVA
library(psych)
tabela_descritiva <- describe(data_for_desc_stat)

# 3. SALVAR
write.csv(tabela_descritiva, "tabela_descritivas.csv", row.names = FALSE)
print(round(tabela_descritiva, 2))

# 4. CORRELAÇÃO (SEM MUDANÇA)
matriz_cor <- cor(data_for_desc_stat, method = "pearson")
write.csv(round(matriz_cor, 2), "matriz_correlacao.csv", row.names = TRUE)

# Exibir a matriz numérica arredondada para 2 casas decimais
print(round(matriz_cor, 2))

# --- Visualização Gráfica (Recomendado) ---
# O corrplot ajuda a identificar padrões rapidamente
corrplot(matriz_cor, 
         method = "color",    # Usa cores para indicar força
         type = "upper",      # Mostra apenas a parte superior
         addCoef.col = "black", # Adiciona o valor numérico
         tl.col = "black",    # Cor do texto das etiquetas
         diag = FALSE)        # Remove a diagonal principal

```

### VIF
```{r}
# 1. Criar um modelo de regressão linear múltipla
# Exemplo: Prever 'mpg' (consumo) usando todas as outras variáveis (.)
modelo <- lm(Empresa_ESG ~ ., data = data_for_desc_stat)
...
# --- Diagnóstico Rápido ---
# Se VIF > 5 ou 10, há alta multicolineariedade
print(vif_valores[vif_valores > 5])
```
# Verificar os limites atuais do df_final (após o script rodar tudo)
summary(df_final$ROE)
summary(df_final$EBITDA_Margin)

# Se os valores de Min/Max ainda estiverem na casa dos milhares (ex: -13.000 ou 4.000), 
# significa que a limpeza não foi aplicada ou o df_final foi sobrescrito por erro.
