...
obter_empresas_esg_por_data <- function(col_alvo, data_ref, lista_sheets) {
  ...
  corte_minimo <- qtd_fundos_ativos_data * 0.20  #1% ou 20% (Change the values here for 0.00001 for Y>=0.01 and 0.2 for Y>= 1)
...
Remove tickers
```{r}
codigo_to_remove <- c("SYNE3", "AMAR3", "RCSL3")
df_final <- filter(df_final, !Código %in% codigo_to_remove)
```
